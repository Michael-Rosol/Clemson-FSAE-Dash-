...
//  TxHeader.Identifier = 0x1; // This is the 11 bit Identifer

  // Configure the TxHeader for Classical CAN
  TxHeader.Identifier = 0x1; // Standard identifier
  TxHeader.IdType = FDCAN_STANDARD_ID; // Use standard ID (11-bit)
  TxHeader.TxFrameType = FDCAN_DATA_FRAME; // Data frame type
  TxHeader.DataLength = FDCAN_DLC_BYTES_8; // Classical CAN allows DLC up to 8 bytes
  TxHeader.ErrorStateIndicator = FDCAN_ESI_ACTIVE; // Error state indicator is active
  TxHeader.BitRateSwitch = FDCAN_BRS_OFF; // Disable Bit Rate Switch for Classical CAN
  TxHeader.FDFormat = FDCAN_CLASSIC_CAN; // Use Classical CAN format
  TxHeader.TxEventFifoControl = FDCAN_NO_TX_EVENTS; // No transmission events
  TxHeader.MessageMarker = 0x0; // Marker not used
  ...
  #if 0
   fdcanfilterconfig.FilterType = FDCAN_FILTER_DUAL; // Ignore because FDCAN_FILTER_TO_RXBUFFER
  #endif
    fdcanfilterconfig.FilterConfig = FDCAN_FILTER_TO_RXBUFFER;
    fdcanfilterconfig.FilterID1 = 0x2; // ID Node2
  #if 0
    fdcanfilterconfig.FilterID2 = 0x0; // Ignore because FDCAN_FILTER_TO_RXBUFFER
  #endif
